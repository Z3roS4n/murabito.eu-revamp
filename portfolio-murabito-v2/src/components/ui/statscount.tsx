...
function AnimatedCounter({
  ...
  return (
    ...
    >
      <motion.div
        className={cn(
          "text-5xl sm:text-5xl md:text-4xl lg:text-5xl font-bold mb-2 sm:mb-4 whitespace-nowrap"
        )}
        initial={{ scale: 0.8 }}
...
export default function StatsCount({
  ...
  return (
    <motion.section
      ref={containerRef}
      className={cn(
        "mt-20 px-2 sm:px-4 md:px-8 w-full overflow-hidden", //py-8 sm:py-12 lg:py-20
        className
      )}
      initial={{ opacity: 0 }}
      animate={isInView ? { opacity: 1 } : { opacity: 0 }}
    ...
    >
      ...
      <div className={cn("w-full max-w-6xl mx-auto")}>       
        <div
          className={cn(
            "flex lg:flex-row flex-col items-stretch justify-between gap-2 sm:gap-4 lg:gap-8 w-full min-h-[120px] sm:min-h-[140px]"
          )}
        >
